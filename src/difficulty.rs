...
impl DifficultyCalculator {
    ...
    pub fn adjust_difficulty_kadena_style(
        ...
        let clamped_ratio = adjustment_ratio.max(min_adjustment).min(max_adjustment);
    ...
    pub fn should_adjust(&self, block_height: u64) -> bool {
        block_height.is_multiple_of(self.config.difficulty_adjustment_interval) && block_height > 0
    }
    ...
    pub fn validate_difficulty(&self, difficulty: u64) -> Result<()> {
        if difficulty < self.config.min_difficulty {
            return Err(PoWError::InvalidDifficulty(format!(
        ...
        if difficulty > self.config.max_difficulty {
            return Err(PoWError::InvalidDifficulty(format!(
                ...
                difficulty, self.config.max_difficulty
            )));
        }
...
mod tests {
    ...
    fn test_calculate_difficulty_faster_blocks() {
        ...
        let new_diff = calc
            .calculate_difficulty(1_000_000, &block_times, 100)
            .unwrap();
...
fn test_bits_to_difficulty_roundtrip() {
    ...
    assert!(
        ...
        "Roundtrip failed: {} -> 0x{:08x} -> {} (ratio: {:.4})",
        original_diff,
        bits,
        recovered_diff,
        ratio
